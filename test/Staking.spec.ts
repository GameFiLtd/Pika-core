...
describe('Staking', () => {
  ...
  before(async () => {
    ...
    erc20 = await TokenFactory.deploy();
    const StakingFactory = await ethers.getContractFactory('Staking');
    staking = await StakingFactory.deploy('Staked Test Token', 'sTST', erc20.address);
    domain = {
      name: 'Test Token',
      version: '1',
  ...
  describe('Stake with permit', async () => {
    ...
    before(async () => {
      ...
      signature = await accounts[3]._signTypedData(domain, types, values);
    });
    ...
    it('should be able to stake with permit', async () => {
      const tx = staking.connect(accounts[3]).depositWithPermit(amount.mul(2), unix2100, signature);
      await expect(tx).to.emit(staking, 'Deposit').withArgs(accounts[3].address, amount.mul(2), amount.div(2));
      expect(await erc20.balanceOf(staking.address)).to.equal(amount.mul(20));
      expect(await staking.balanceOf(accounts[3].address)).to.equal(amount.div(2));
      expect(await staking.totalSupply()).to.equal(amount.mul(5));
      expect(await staking.tokenBalance(accounts[3].address)).to.equal(amount.mul(2));
    });
  });
  ...
  describe('Snapshots', async () => {
    it('only accounts with snapshotter role should be able to take snapshots', async () => {
      await expect(staking.connect(accounts[1]).takeSnapshot()).to.be.revertedWith(
        'AccessControl: account ' +
          accounts[1].address.toLowerCase() +
          ' is missing role ' +
          (await staking.SNAPSHOTTER_ROLE())
      );
      await staking.grantRole(await staking.SNAPSHOTTER_ROLE(), accounts[1].address);
      await expect(staking.connect(accounts[1]).takeSnapshot()).to.emit(staking, 'Snapshot');
    });
  });
});
